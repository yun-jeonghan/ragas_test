...
- GREV_KGGEN_MINE_PROVIDER
- GREV_KGGEN_MINE_MODEL
- GREV_KGGEN_MINE_BASE_URL
- GREV_KGGEN_MINE_API_KEY
- GREV_KGGEN_MINE_EXTRA_BODY
- GREV_KGGEN_MINE_MAX_TOKENS

이쪽은 retrieved context가 reference answer를 지지하는지 binary로 판정하는 judge입니다.
기본값은 Ragas / BenchmarkQED와 같은 로컬 Ollama 계열을 따릅니다.
`GREV_KGGEN_MINE_EXTRA_BODY` 역시 chat/completions 요청에만 병합됩니다.
`GREV_KGGEN_MINE_MAX_TOKENS` 도 기본값이 256이라서 짧게 판정만 하도록 둡니다.
...
- 새로 시작하는 경우에는 GREV_RAGAS_* 와 GREV_BENCHMARKQED_*,
  그리고 필요하면 GREV_KGGEN_MINE_* 만 쓰는 편이 좋습니다.
